...
/** hostel is "false" for PARENT in the frontend's access matrix, and there's no per-student
 * lookup here for a parent to legitimately use, so every list endpoint that names a pupil
 * (allocations, leaves) is staff-only. Room metadata alone (getRooms) doesn't name any pupil,
 * so it's left open, matching fee-structures/levies elsewhere. */
...
public class HostelController {
    ...
    private void requireStaff(Authentication auth) {
        if ("PARENT".equals(roleOf(auth))) {
            throw new ForbiddenException("Your role cannot access hostel records");
        }
    }
    ...
    @GetMapping("/allocations") public ResponseEntity<ApiResponse<List<HostelAllocation>>> getAllocations(@PathVariable String schoolId, Authentication auth) { requireStaff(auth); return ResponseEntity.ok(ApiResponse.ok(hostelService.getAllocations(schoolId))); }
    ...
    @GetMapping("/leaves") public ResponseEntity<ApiResponse<List<HostelLeave>>> getLeaves(@PathVariable String schoolId, Authentication auth) { requireStaff(auth); return ResponseEntity.ok(ApiResponse.ok(hostelService.getLeaves(schoolId))); }
