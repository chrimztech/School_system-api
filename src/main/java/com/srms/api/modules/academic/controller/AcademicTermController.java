...
public class AcademicTermController {
    ...
    private void requireCanManage(String schoolId, Authentication auth) {
        if (!moduleAccessService.isAllowed(schoolId, auth, "settings", "full", CAN_MANAGE_ROLES.contains(roleOf(auth)))) {
    ...
    public ResponseEntity<ApiResponse<AcademicTerm>> create(@PathVariable String schoolId, @RequestBody AcademicTerm dto, Authentication auth) {
        requireCanManage(schoolId, auth);
